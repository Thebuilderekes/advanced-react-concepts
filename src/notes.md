...
## useState hook
State is a way for React to keep track the value of variables through out a React application. This is done using the useState hook. When the value of a variable changes, React will check for the new value and update the UI arcordingly. This also means that if a parent componentn that has a changing variable is updated, a child componentn that also uses that variable is also updated and thi causes a re-rendering of the UI.
...
When lifting state up to a parent componene, remember to use placeholder props in the children components from where the state will be used. As in the case of component in the folder `lifting_state` that contains a seacrh bar component and a component that renders what is searched. You can see that `onSearchUpdate` and `currentTerm` are used as the placeholder for the components which are later given intended values during usage at app component where the satte has been lifted. 
